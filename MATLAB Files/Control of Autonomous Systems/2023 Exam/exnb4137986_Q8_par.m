...
%Q7 Feedback Linearizing
disp(' ')
disp(' ')
disp('Feedback Linearizing')

% Perhaps choose low-value poles?
lambda_cl = [-10, -11]

A_fblin = [lambda_cl(1), lambda_cl(2);
     q2/J, q3/J]
A_fblin = double(subs(A_fblin, params, values));
...
n = size(A_fblin, 1); % Number of states
...
ControllabilityMatrix = ctrb(A_fblin, B) % Use MATLAB's built-in function

% Check rank
rankC = rank(ControllabilityMatrix);
