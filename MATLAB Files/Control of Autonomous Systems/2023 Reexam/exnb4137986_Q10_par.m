...
% Q9 OBSERVABILITY
% Define the output equation y(t) = I(t)
y = X(1)  % Output is the current I

% Output matrix C (dy/dx)
C = jacobian(y, X)

% Initialize observability matrix W0 with C
W0 = C;

% Compute the observability matrix for each step
for i = 1:n-1
    W0 = [W0; C * (A^i)];  % Multiply C with A^i and append
end
...
% Desired observer poles (all at -25)
p_obs_des = -25 * ones(n, 1);

% Compute observer gain L
L = acker(A', C', p_obs_des)';

% Verify the eigenvalues of the error dynamics matrix
eig_error_dynamics = eig(A - L * C);

% Display results
disp('Observer gain L:');
disp(L);
disp('Eigenvalues of the error dynamics:');
disp(eig_error_dynamics);